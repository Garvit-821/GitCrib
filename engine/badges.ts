import { GitHubDataPayload, AchievementBadge } from './types.js';

export function calculateBadges(payload: GitHubDataPayload): AchievementBadge[] {
  ...
  const getBadge = (
    ...
    let level: 'gold' | 'silver' | 'bronze' | 'none' = 'none';
    if (value >= goldThreshold) level = 'gold';
    else if (value >= silverThreshold) level = 'silver';
    else if (value >= bronzeThreshold) level = 'bronze';
  ...
  badges.push(
    getBadge(
      'consistency-king',
      'Consistency King',
      'Longest contribution streak of {val} days',
      60,
      30,
      14,
  ...
  badges.push(
    getBadge(
      'community-favorite',
      'Community Favorite',
      'Earned {val} stars across repositories',
      500,
      150,
      30,
      metrics.totalStars
  ...
  badges.push(
    getBadge(
      'open-source-hero',
      'Open Source Hero',
      'Contributed to {val} external repositories',
      10,
      4,
      1,
      metrics.reposContributedTo
    )
  );

  // 4. Polyglot Developer (Unique Languages)
  const uniqueLanguages = new Set(
    repos.map(r => r.language).filter((l): l is string => typeof l === 'string' && l !== '')
  );
  badges.push(
    getBadge(
      'polyglot-developer',
      'Polyglot Developer',
      'Built projects in {val} different languages',
      7,
      4,
      2,
  ...
  badges.push(
    getBadge(
      'top-contributor',
      'Top Contributor',
      'Committed {val} times over the last year',
      1000,
      300,
      50,
      metrics.commits
  ...
  badges.push(
    getBadge(
      'speed-coder',
      'Speed Coder',
      'Committed an average of {val} times per day',
      3.0,
      1.0,
      0.2,
      metrics.avgCommitsFreq
    )
  );
  ...
  badges.push(
    getBadge(
      'night-owl',
      'Night Owl',
      'Logged {val} late-night commits (10 PM - 4 AM)',
      150,
      60,
      15,
      nightCommits
  ...
  badges.push(
    getBadge(
      'early-bird',
      'Early Bird',
      'Logged {val} early-morning commits (5 AM - 9 AM)',
      120,
      45,
      10,
      morningCommits
  ...
  badges.push(
    getBadge(
      'code-writer',
      '10K Lines Written',
      'Accumulated {val} KB of codebase size',
      20000,
      8000,
      1500,
      totalRepoSizeKB
  ...
  badges.push(
    getBadge(
      ...
      'Created {val} public repositories',
      30,
      15,
      5,
      metrics.publicRepos
  ...
  badges.push(
    getBadge(
      'issue-hunter',
      'Issue Hunter',
      'Participated in {val} discussions or closed issues',
      15,
      7,
      2,
      metrics.discussions + metrics.prsClosed
    )
  );
  ...
  badges.push(
    getBadge(
      'pr-master',
      'PR Master',
      'Successfully merged {val} Pull Requests',
      40,
      15,
      3,
      metrics.prsMerged
