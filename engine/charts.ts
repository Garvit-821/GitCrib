...
export function renderHeatmapGrid(
  ...
  const getFill = (val: number) => {
    if (val === 0) return colors.cardBg;
    if (val < 3) return colors.accent2 + '3a'; // 25% opacity
    if (val < 6) return colors.accent2 + '77'; // 50% opacity
    if (val < 10) return colors.accent2 + 'cc'; // 80% opacity
