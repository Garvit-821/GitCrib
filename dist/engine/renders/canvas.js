...
import { renderBarChart, renderRadarChart, renderHeatmapGrid } from '../charts.js';
 ...
 */
export function drawToCanvas(ctx, // CanvasRenderingContext2D
payload, type, themeName = 'blueprint') {
    ...
    const drawPanel = (id, drawContent) => {
        const box = layout[id];
        if (!box || box.x < 0)
            return;
    ...
    drawPanel('barChart', (cx, cy, w, h) => {
        ...
        bars.forEach((b, idx) => {
            ...
            ctx.fillStyle = '#666666';
            ctx.font = '8px monospace';
            ctx.fillText(['A', 'S', 'O', 'N', 'D', 'J', 'F', 'M', 'A', 'M', 'J', 'J'][idx], cx + b.x + b.w / 2 - 3, cy + 186);
        });
    });
    // 5. Donut / Languages
