 ...
 */
export function getViewportConfig(type) {
    switch (type) {
        case 'instagram-post':
 ...
 */
export function computeLayout(type) {
    const layout = {};
    if (type === 'poster') {
        ...
        layout['repos'] = { id: 'repos', x: 1900, y: 60, w: 1880, height: 550, title: 'Top Repos' };
        layout['badges'] = { id: 'badges', x: 1900, y: 640, w: 1880, height: 370, title: 'Achievements' };
    ...
    if (layout['repos'] && !layout['repos'].width)
        layout['repos'].width = layout['repos'].w;
    if (layout['badges'] && !layout['badges'].width)
        layout['badges'].width = layout['badges'].w;
    return layout;
}
/**
 * Scan GitHubDataPayload for tech keywords in repo names/languages/descriptions
 */
export function detectTechStack(payload) {
 ...
 */
export function computeInsights(payload) {
    const repos = payload.repos;
